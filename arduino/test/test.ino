...
// ------------------- Robot Parameters -----------------------
const float WHEEL_BASE    = 0.26;   // effective track width for walking mechanism (m)
...
void driveMotors(float linear, float angular) {
  ...
  float targetL = constrain(vLeft  / MAX_SPEED, -1.0, 1.0) * LEFT_MOTOR_SCALE;
  float targetR = constrain(vRight / MAX_SPEED, -1.0, 1.0) * RIGHT_MOTOR_SCALE;
  ...
  setMotor(enA, in1, in2, smoothL);
  setMotor(enB, in3, in4, smoothR);
}
